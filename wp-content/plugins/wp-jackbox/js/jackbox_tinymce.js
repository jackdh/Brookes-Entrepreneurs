(function($) {
		...
		container = '<div class="jackbox-modal">' +
			...
			'<div class="jackbox-cover">' +
				...
				'<div class="jackbox-container">' +
					...
					'<input class="jackbox-close" type="submit" value="x" />' +
					...
					'<div class="jackbox-content">' + 
						...
						'<form class="jackbox-form" method="post" enctype="text/plain">' +
							...
							'<hr /><h3>' + jackboxTranslate["jb_9"] + '</h3>' +
	...
	function startIt(ed, url) {
		...
		editor.addButton("jackbox", {

			title: "Add/Edit JackBox",
			image: path + "../img/jackbox.png",
			onclick: openModal

		});

		editor.addButton("jackbox_remove", {

			title: "Remove JackBox",
			image: path + "../img/jackbox_remove.png",
			onclick: removeJackbox
	...
	function addModal() {
		...
		$("#jb-tiny-desc_parent").css({display: "block", marginTop: 7}).find(".mce_jackbox, .mce_jackbox_remove").remove();

		htmlFrame = $("#jb-tiny-html_ifr").css({backgroundColor: "#FFF", height: 215}).hide();
		htmlTiny = $("#jb-tiny-html_parent").css({display: "block", paddingTop: 7, clear: "both"}).hide();
		htmlTiny.find(".mce_jackbox, .mce_jackbox_remove").remove();
	...
	function openModal() {
		...
		for(i = 0; i < leg; i++) {
			...
			if(cats.indexOf(str) === -1) cats[cats.length] = str;
		...
		if(cats.length) {
			...
			if(grouped.val()) {
				...
				if(i !== -1) extras.children("input[name=jb-" + cats[i] + "]").attr("checked", "checked");

			}
			else {

				grouped.val(cats[0]);
				extras.children("input[name=jb-" + cats[0] + "]").attr("checked", "checked");
		...
		if(this.checked) {
			...
			while(i--) {
				...
				if(child[0] !== this) {
					...
					grouped.val(child.attr("name").split("jb-")[1]);
	...
	function checkGroup() {
		...
		if(i === -1) {
			...
			extras.children("input[name=jb-" + cats[i] + "]").attr("checked", "checked");
	...
	function submitData(event) {
		...
		(node.is("a")) ? itm.html(node.html()) : itm.append(node);
	...
	function writeAttributes() {
		...
		if(type !== "radio") {
			...
			if(namer === "title") value = escape(value);
	...
	function updateMedia(html) {
		...
		var input, url = $("img", html).attr("src"), holder;
		if(!url) url = $(html).attr("href");
